...
struct Buckets {
    ...
    void putL(T c, int pos) {
        buf[idx[c].l_cur++] = pos;
    }
    void putS(T c, int pos) {
        buf[idx[c].s_cur--] = pos;
    }
...
class SAIS {
 ...
 public:
    SAIS(const char* str)
            : m_buf_len(strlen(str) + 1),
              m_bucket_buf(new int[m_buf_len]) {
 ...
 private:
    ...
    bool lms_eql(const T* str, const SLtypes& types, unsigned i1, unsigned i2) const {
        for (;; i1++, i2++)
            if (str[i1] != str[i2] || types[i1] != types[i2])
                return false;
            else if (isLMS(i1 + 1, types) && isLMS(i2 + 1, types))
